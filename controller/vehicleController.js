...
module.exports = {
  ...
  updateVehicleSchedule:async(req,res)=>{
    try {
      ...
      if (!vehicleSchedule) {
        return res.status(404).json({ success: false, message: "Vehicle Schedule not found" });
      }
      ...
      if (vehicleNumber && vehicleNumber !== vehicleSchedule.vehicleNumber) {
        const existingVehicle = await VehicleSchedule.findOne({ where: { vehicleNumber } });
        if (existingVehicle) {
      ...
      return res.status(200).json({ success: true, message: "Vehicle Schedule updated successfully", data: vehicleSchedule });
    } catch (error) {
      console.error("Error updating vehicle schedule:", error);
      return res.status(500).json({ success: false, message: "Error updating vehicle schedule", error });
    }
  },
  deleteVehicleSchedule:async(req,res)=>{
    try {
      ...
      return res.status(200).json({ success: true, message: "Vehicle Schedule moved to trash successfully" });
    } catch (error) {
      console.error("Error deleting vehicle schedule:", error);
      return res.status(500).json({ success: false, message: "Error deleting vehicle schedule", error });
    }
  },
  ...
  getVehicleSchedules:async(req,res)=>{
    try {
      ...
      return res.status(500).json({ success: false, message: "Error fetching vehicle schedules", error });
    }
  },
  getVehicleScheduleById:async(req,res)=>{
    try {
      ...
      return res.status(500).json({ success: false, message: "Error fetching vehicle schedule", error });
    }
  },
  ...
  updateVehicleServiceProvider:async(req,res)=>{
    try {
      ...
      if (!vehicleService) {
        ...
        return res.status(404).json({ success: false, message: "Vehicle Service not found" });
      }
      ...
      return res.status(200).json({ success: true, message: "Vehicle Service updated successfully", data: vehicleService });
  
    } catch (error) {
      await deletefilewithfoldername(uploadPath,req.files.image[0].filename)
      await deletefilewithfoldername(uploadPath,req.files.icon[0].filename)
      console.error("Error updating vehicle service:", error);
      return res.status(500).json({ success: false, message: "Error updating vehicle service", error });
    }
  },
  deleteVehicleServiceProvider:async(req,res)=>{
    try {
      ...
      return res.status(200).json({ success: true, message: "Vehicle Service deleted successfully" });
  
    } catch (error) {
      console.error("Error deleting vehicle service:", error);
      return res.status(500).json({ success: false, message: "Error deleting vehicle service", error });
    }
  },
  ...
  getVehicleServiceProviders:async(req,res)=>{
    try {
      ...
      return res.status(500).json({ success: false, message: "Error fetching vehicle services", error });
    }
  },
  getVehicleServiceProviderById:async(req,res)=>{
    try {
      ...
      return res.status(500).json({ success: false, message: "Error fetching vehicle service", error });
    }
  }
};
